...
struct Aabb(int _D, _F) if(2 <= _D && _D <= 3 && isAabbType!_F)
{
    ...
    void expand(Vec_ p)
    {
        static foreach(x; 0..D) if(p[x] > f1[x]) f1[x] = p[x]; else if(p[x] < f0[x]) f0[x] = p[x];
    }
    
    void expand(Aabb_ o)
    {
        static foreach(x; 0..D) { if(o.f1[x] > f1[x]) f1[x] = o.f1[x]; if(o.f0[x] < f0[x]) f0[x] = o.f0[x]; }
    }
    
    bool intersects(Aabb_ o)
    {
        static foreach(x; 0..D) if((f0[x] > o.f1[x]) || (o.f0[x] > f1[x])) return false;
        return true;
    }
    
    bool contains(Vec_ p)
    {
        static foreach(x; 0..D) if((f0[x] > p[x]) || (p[x] > f1[x])) return false;
        return true;
    }
    
    bool contains(Aabb_ o)
    {
        static foreach(x; 0..D) if((f0[x] > o.f0[x]) || (o.f1[x] > f1[x])) return false;
        return true;
    }
}
